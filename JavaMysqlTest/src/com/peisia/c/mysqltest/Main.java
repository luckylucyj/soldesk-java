...
public class Main {
	...
	static private void initDb() {
		try {
			Class.forName("com.mysql.cj.jdbc.Driver");//로직으로 이해하면 됨-pm 4:18
	...
	static private void dbRun() {
		try { // 아래 코드들을 이 try{ 블럭 안에 안쓰면 에러남.
				// *db명* *id* *암호*
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/my_cat", "root", "root");// 내부적으로
																												// 객체를
																												// 생성해서
																												// 반환해주는
																												// 객체-pm
			...
			//java.sql.Statement를 Statement로 할 수 있긴 한데 내부객체가 아니기 때문에 에러가 뜬다-pm 4:24

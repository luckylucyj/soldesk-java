<%@page import="com.homecok.board.BoardListProcessor"%>
<%@page import="com.homecok.board.Board"%>
...
<body>
	...
	<fieldset>
		...
		<%=posts.get(i).no%>
		<a
			href="/board/read?category=<%=blp.category%>&no=<%=posts.get(i).no%>">
			<%=posts.get(i).title%>
		</a>
		<%=posts.get(i).id%>
		<hr>
	...
	<fieldset>
		...
		<a href="/board/list?category=<%=blp.category%>">list로</a> 
		<a href="/">홈으로</a>
	</fieldset>
	<fieldset>
		<legend>검색</legend>
